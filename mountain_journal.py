...
# Function to calculate duration and generate climb summary
def generate_climb_summary(climb_dir, climb):
    start_time = datetime.fromisoformat(climb["start_time"])
    end_time = datetime.fromisoformat(climb["end_time"])
    duration = end_time - start_time
    
    # Placeholder values for distance and elevation gain
    distance_traveled = "N/A"  # To be replaced with real GPS data
    elevation_gain = "N/A"     # To be replaced with real elevation data

    # Count journal entry types
    total_entries = len(climb["entries"])
    text_count = sum(1 for entry in climb["entries"] if entry["type"] == "text")
    audio_count = sum(1 for entry in climb["entries"] if entry["type"] == "audio")
    image_count = sum(1 for entry in climb["entries"] if entry["type"] == "image")

    # Format the summary
    summary = (
        f"Climb Summary for: {os.path.basename(climb_dir)}\n"
        f"Duration: {str(duration).split('.')[0]}\n"  # Remove milliseconds
        f"Distance Traveled: {distance_traveled}\n"
        f"Elevation Gain: {elevation_gain}\n"
        f"Total Number of Journal Entries: {total_entries}\n"
        f"Text Entries: {text_count}\n"
        f"Audio Entries: {audio_count}\n"
...
def save_climb_summary(climb_dir, summary):
    summary_file = os.path.join(climb_dir, "climb_summary.txt")
    with open(summary_file, "w") as f:
        f.write(summary)
    print(f"Climb summary saved as {summary_file}")
...
def end_climb():
    ...
    climb_dir = os.path.dirname(active_climb)
    print(f"Climb '{climb_dir}' ended at {end_time}, location: {end_location}")

    # Generate and save the climb summary
    summary = generate_climb_summary(climb_dir, climb)
    save_climb_summary(climb_dir, summary)
